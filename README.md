...
**一种读取 JSON 文档的 Java DSL。**

[![Build Status](https://travis-ci.org/json-path/JsonPath.svg?branch=master)](https://travis-ci.org/json-path/JsonPath)
[![Maven Central](https://maven-badges.herokuapp.com/maven-central/com.jayway.jsonpath/json-path/badge.svg)](https://maven-badges.herokuapp.com/maven-central/com.jayway.jsonpath/json-path)
[![Javadoc](https://www.javadoc.io/badge/com.jayway.jsonpath/json-path.svg)](http://www.javadoc.io/doc/com.jayway.jsonpath/json-path)

Jayway JsonPath[Stefan Goessner JsonPath implementation](http://goessner.net/articles/JsonPath/)的 Java 版移植

新闻
----
10 Dec 2020 - 发布 JsonPath 2.5.0

05 Jul 2017 - 发布 JsonPath 2.4.0

26 Jun 2017 - 发布 JsonPath 2.3.0

29 Feb 2016 - 发布 JsonPath 2.2.0

22 Nov 2015 - 发布 JsonPath 2.1.0

19 Mar 2015 - 发布 JsonPath 2.0.0

11 Nov 2014 - 发布 JsonPath 1.2.0

01 Oct 2014 - 发布 JsonPath 1.1.0  

26 Sep 2014 - 发布 JsonPath 1.0.0 


入门
---------------

JsonPath 可以在 Maven 中央仓库获取。 Maven 用户添加以下内容到你的 POM。
...
如果你需要帮助，在 [Stack Overflow](http://stackoverflow.com/questions/tagged/jsonpath) 提问。 问题标签为 'jsonpath' 和 'java'。

JsonPath 表达式总是指向一个 JSON 结构，正如 XPath 表达式与 XML 文档一起使用的方式。JsonPath 中的 “根成员对象” 由 `$` 表示，无论这是一个对象还是一个数组。
...
| 操作符                    | 描述                                                               |
| :------------------------ | :----------------------------------------------------------------- |
| `$`                       | 待查询的根元素。所有路径表达式都以此开头                           |
| `@`                       | 当前被过滤器判断式处理的节点                                       |
| `*`                       | 通配符。可用于任何需要名称或数字的位置                             |
| `..`                      | 深度扫描。 可用于任何需要名称的位置                                |
| `.<name>`                 | 点表示法的子元素                                                   |
| `['<name>' (, '<name>')]` | 方括号表示法的一个多个子元素                                       |
| `[<number> (, <number>)]` | 数组的一个或多个索引                                               |
| `[start:end]`             | 数组切分操作符                                                     |
| `[?(<expression>)]`       | 过滤器表达式。表达式必须取值为一个布尔值                           |


函数
---------

函数可以在路径尾端被调用 - 路径表达式的输出是函数的输入。
函数的输出从函数本身可以推断出来。

| 函数                      | 描述                                                                | 输出类型    |
| :------------------------ | :------------------------------------------------------------------ |:----------- |
| min()                     | 提供一个数字数组中的最小值                                          | Double      |
| max()                     | 提供一个数字数组中的最小值                                          | Double      |
| avg()                     | 提供一个数字数组中的平均值                                          | Double      | 
| stddev()                  | Provides the standard deviation value of an array of numbers        | Double      | 
| length()                  | Provides the length of an array                                     | Integer     |
| sum()                     | Provides the sum value of an array of numbers                       | Double      |
| keys()                    | Provides the property keys (An alternative for terminal tilde `~`)  | `Set<E>`    |
